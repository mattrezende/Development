...
const TeacherBanner = ({ teacher }) => {
  ...
  const bannerUrl = apiClient.fileUrl(teacher?.bannerImagePath);
  const avatarUrl = apiClient.fileUrl(teacher?.profilePhotoPath);
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          <div className="lg:col-span-2">
            ...
            <p className="text-muted-foreground leading-relaxed text-balance">
              {teacher?.professionalDescription || "O professor ainda não adicionou uma descrição profissional."}
            </p>
          </div>
          ...
          <div className="bg-muted/30 rounded-2xl p-5 border border-border space-y-4">
            ...
            {teacher?.baseCity && (
              <div className="flex items-center gap-3 text-foreground/90">
                ...
                <span className="text-sm font-medium">{teacher.baseCity}</span>
              </div>
            )}
            ...
            {teacher?.contactPhone && (
              <div className="flex items-center gap-3 text-foreground/90">
                ...
                <span className="text-sm font-medium">{teacher.contactPhone}</span>
              </div>
            )}
            ...
            {teacher?.contactEmail && (
              <div className="flex items-center gap-3 text-foreground/90">
                ...
                <span className="text-sm font-medium">{teacher.contactEmail}</span>
              </div>
            )}
          </div>
        </div>
