...
const TimesheetManagement = () => {
  ...
  const fetchTimesheets = async () => {
    try {
      ...
      if (startDate && endDate) {
        ...
        filter += ` && date >= "${startDate}" && date < "${nextDayStr}"`;
      }

      const records = await pb.collection('timesheets').getFullList({
        filter,
        sort: '-date',
        expand: 'employee_id,manager_id',
        $autoCancel: false
      });
  ...
  const getStatusBadge = (status) => {
    const variants = {
      pending: { variant: 'default', label: 'Pendente', className: 'bg-amber-500 hover:bg-amber-600' },
      approved: { variant: 'default', label: 'Aprovado', className: 'bg-emerald-500 hover:bg-emerald-600' },
      rejected: { variant: 'destructive', label: 'Rejeitado', className: '' }
    };
  ...
  return (
    <>
      <Helmet>
        <title>Gestão de Ponto - Sistema de Gestão de RH</title>
        <meta name="description" content="Aprove e gerencie registros de ponto dos funcionários" />
      </Helmet>
      ...
      <div className="min-h-screen bg-gray-50">
        ...
        <div className="container mx-auto px-4 py-8">
          <div className="mb-8">
            <h1 className="text-3xl font-bold text-gray-900">Gestão de Ponto</h1>
            <p className="text-gray-600 mt-2">Aprove ou rejeite registros de ponto dos funcionários</p>
          </div>
